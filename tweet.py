...
def main():
    ...
    if csv:
        ...
        summary_text = f"{df.loc[0, 'name']} tops the table with {df.loc[0, 'map_changes']},followed by {df.loc[1, 'name']} with {df.loc[1, 'map_changes']} and {df.loc[2, 'name']} with {df.loc[2, 'map_changes']} map changes."
    ...
    if args.tweet_last_week:
        if args.tweet_global:
            api.update_status(
                status=f"Global Contributors Last Week (UTC Timezone)\n(From {lstfile[1]} to {lstfile[2][:-4]})\n{summary_text}\nFull stats: https://github.com/kshitijrajsharma/OSMSG/tree/master/stats/Global/Weekly/weekly_global_stats.csv #weeklystats #OpenStreetMap #global",
                media_ids=[media.media_id],
            )

        else:
            api.update_status(
                status=f"Nepal Contributors This week (UTC Timezone)\n(From {lstfile[1]} to {lstfile[2][:-4]})\n{summary_text}\nCheck full stats: https://github.com/kshitijrajsharma/OSMSG/tree/master/stats/Nepal/Weekly/weekly_nepal_stats.csv #weeklystats #OpenStreetMap #osmnepal",
                media_ids=[media.media_id],
            )
        print("twitted")
